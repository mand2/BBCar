...
<!-- tmap -->
<script
	src="https://apis.openapi.sk.com/tmap/js?version=1&format=javascript&appKey=5beda631-7db0-4be9-b0bd-b6b5a7f41945"></script>
<!-- bootstrap -->
<script
...
<body data-spy="scroll" data-target=".site-navbar-target"
	...
	<div class="container">
		...
		<div class="row">
			<div class="col-md-12">
				<div class="search-wrap-1 ftco-animate p-4 boxes">
					<button id="confirmRsv" onclick="confirmRsv();"
						class="btn btn-primary rsvsbtn">확정된예약</button>
					<button id="waitingRsv" onclick="waitingRsv();"
						class="btn btn-primary rsvsbtn">대기예약</button>
					<button id="waitingRsv" onclick="pastRsv();"
						class="btn btn-primary rsvsbtn">지난예약</button>
				</div>
				<div class="boxes">
		...
		<div class="row justify-content-center pb-0 pb-mb-5 pt-5 pt-md-0">
			<input type="hidden" value="${sessionScope.login.nickname}"
				id="chatNickName">
		</div>
		<h1 class="mb-4">채팅</h1>
	...
	var p_idx = ${sessionScope.login.idx};
		...
		function confirmRsv() {
			...
			$
					.ajax({
						url : 'http://13.125.252.85:8080/server/rsv/mycarpool/'
								+ p_idx,
						type : 'GET',
						success : function(data) {
							...
							for (var i = 0; i < data.length; i++) {
								...
								html += '운전자 닉네임 ' + data[i].nickname
										+ '<br>\n';
								html += '카풀일시 ' + data[i].d_date + '\t'
										+ data[i].d_starttime + '\t -\t '
										+ data[i].d_endtime + '<br>\n';
								html += '출발지 ' + data[i].d_startpoint
										+ '<br>\n';
		...
		function waitingRsv() {
			...
			$
					.ajax({
						url : 'http://13.125.252.85:8080/server/rsv/waitcarpool/'
								+ p_idx,
						type : 'GET',
						success : function(data) {
							var html = '';
							for (var i = 0; i < data.length; i++) {
								...
								html += '카풀일시 ' + data[i].d_date + '\t'
										...
										+ data[i].d_endtime + '<br>\n';
								html += '출발지 ' + data[i].d_startpoint
										+ '\t 도착지 \t ' + data[i].d_endpoint
										+ '<br>\n';
								html += '요금 ' + data[i].d_fee + '원 <br>\n';
								html += '<button id="delete" onclick="deleteReq('
										+ p_idx
										+ ','
										+ data[i].r_idx
										+ ')" class="btn btn-primary" data-toggle="modal" data-target="#deleteModal">요청취소</button>';
		...
		function pastRsv() {
			...
			$.ajax({
				url : 'http://13.125.252.85:8080/server/rsv/pastcarpool/'
						+ p_idx,
				type : 'GET',
				success : function(data) {
					var html = '';
	...
	<script>
		...
		function fn_chat(r_idx) {
			...
			socketChat.emit('set room', {
				room : r_idx,
				nickname : chat_nickname
			});
			//title 재정의
			document.getElementById('roomTitle').innerHTML = r_idx + '번 방 개설!'
					+ '<input type="hidden" id="chatIdx" value = "'+r_idx+'">';
